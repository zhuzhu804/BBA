...
def cal_acc_cdd(loader, netF, netB, netC, flag=False):
    ...
    centers = torch.zeros(8, 512).cuda()  # Assuming 512-dimensional feature space, adjust if needed
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            # Collect features for each class (needed for CCD)
            for i in range(len(labels)):
                features_by_class[labels[i].item()].append(outputs[i].cpu().numpy())
    ...
    for class_id in range(8):  # Assuming 8 classes
        ...
        centers[class_id] = class_features.mean(dim=0)
    ...
    for i in range(8):
        sum_intra = intra_class_compactness[i]
        sum_inter = 0
        for j in range(8):
            if i != j:
                sum_inter += inter_class_distance[i, j]
        ccd_per_class[i] = sum_intra / sum_inter

    # Calculate average CCD
    avg_ccd = torch.mean(ccd_per_class).item()
    ccd_values = ccd_per_class.cpu().numpy().tolist()
    print('avg_ccd', avg_ccd)
    print('ccd_values', ccd_values)
    ...
    while iter_num < max_iter:
        ...
        if iter_num % interval_iter == 0 or iter_num == max_iter:
            ...
            if args.dset == 'VISDA-C':
                ...
                acc_s_te, _ = cal_acc_cdd(dset_loaders['test'], netF, netB, netC, False)
...
if __name__ == "__main__":
    ...
    for i in range(len(names)):
        ...
        args.output_dir_t = osp.join(args.output_t, args.da, args.dset,
