...
def data_load(args): 
    ...
    txt_tar = open(path + f'{args.names[args.t]}_LDL/{args.names[args.t]}_ldl_test.txt').readlines()
...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            # if i == len(loader) - 1:
            #     break
            loss = F.kl_div(torch.log(outputs.softmax(-1)), labels, reduction='batchmean')
            losses.append(loss)
            # if start_test:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dset', type=str, default='Emo_Rec', choices=['VISDA-C', 'office', 'office-home', 'office-caltech','Emo_Rec','Emo_ldl'])
