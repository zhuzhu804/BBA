...
def cal_acc(loader, netF, netB, netC, flag=False):
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            loss_bc = bhattacharyya_coefficient(softmax_outputs, labels)
            loss_cos = cosine_similarity(outputs, labels, dim=1).mean()
            loss_canb = canberra_distance(softmax_outputs,labels)
            loss_che = chebyshev_distance(outputs,labels)

            losses_kl.append(loss_kl)
            losses_ssd.append(loss_ssd)
            losses.append(loss)
            losses_bc.append(loss_bc)
            losses_cos.append(loss_cos)
            losses_canb.append(loss_canb)
            losses_che.append(loss_che)


    accuracy = sum(losses) / len(losses)
    acc_ssd = sum(losses_ssd) / len(losses_ssd)
    acc_kl = sum(losses_kl) / len(losses_kl)
...
def train_source(args):
    ...
    while iter_num < max_iter:
        ...
        # classifier_loss = our_CrossEntropy()(outputs_source, labels_source)
